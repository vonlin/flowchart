...
jQuery.fn.contextPopup = function(menuData) {
  var menuData = {
    items: [
      ...
      children:{
        ...
        items: [
          {label:'My Item', 
          icon:'/some/icon1.png', action:function() { alert('hi'); }},
          {label:'Item #2', icon:'/some/icon2.png', action:function() { alert('yo'); }},
          null, // divider
          {label:'Blahhhh', icon:'/some/icon3.png', action:function() { alert('bye'); }},
        ]
	...
	// merge them
	let settings = $.extend(true,{},defaultSettings, menuData);

  // Build popup menu HTML
  function createMenu(settings,e) {
    var menu = $('<ul '+(settings.isLeaf ? 'isleaf' : '')+' class="' + settings.contextMenuClass + '"><div class="' + settings.gutterLineClass + '"></div></ul>')
    if(!settings.isLeaf){
    ...
    settings.items.forEach(function(item) {
      if (item) {
        var rowCode = `<li ${item.children ? 'hasChildren' : ''}><a href="#"><span></span></a></li>`;
        // if(item.icon)
        //   rowCode += '<img>';
        ...
        if (item.action) {
          row.find('a').click(function(e1){
            e1.stopPropagation()
            if(item.children){
                let child = createMenu($.extend(true,{},defaultSettings, item.children),e)
                row.append(child)
            }else{
              removeAllPopup()
              item.action(e); 
  ...
  $('body').on('click contextmenu',removeAllPopup)

  // On contextmenu event (right click)
  this.bind('contextmenu', function(e) {	
    removeAllPopup()
    var menu = createMenu(settings,e)
      .show();
    ...
    menu.find('a').click(function(e) {
      // bg.remove();
      e.stopPropagation()
      if(!$(this).closest('li')[0].hasAttribute('hasChildren')){
        removeAllPopup()
      }
    });
